...
/* Always-visible fallback so garage never shows FAIL with nothing */
bool meshLoadCube(Mesh* mesh, float cr, float cg, float cb) {
	...
	for (int i = 0; i < 12; i++) {
		for (int k = 0; k < 3; k++) {
			...
			mv.r = cr; mv.g = cg; mv.b = cb; mv.a = 1.f;
			tris.push_back(mv);
		}
	}
...
bool meshLoadOBJ(const char* path, Mesh* mesh, float cr, float cg, float cb) {
	...
	if (!f) {
		/* try without romfs prefix */
		const char* slash = strrchr(path, '/');
		if (slash) f = fopen(slash + 1, "rb");
	}
	if (!f) {
		/* fallback cube so 3D still shows */
		return meshLoadCube(mesh, cr, cg, cb);
	}
	...
	while (fgets(line, sizeof(line), f)) {
		if (line[0] == 'v' && line[1] == ' ') {
			...
			else if (strstr(line, "metallic") || strstr(line, "chrome")) {
				r = 0.55f; g = 0.55f; b = 0.6f; a = 1.0f;
