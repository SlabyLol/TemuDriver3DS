...
static void drawCar3D(float angleY, float posX, float posY, float posZ, float scale) {
	...
	Mtx_PerspTilt(&projection, C3D_AngleFromDegrees(45.0f), 400.0f/240.0f, 0.1f, 100.0f, false);
...
static void spawnObs() {
	for (int i = 0; i < MAX_OBS; i++) if (!obs[i].on) {
		obs[i].on = true; obs[i].z = 80 + (rand() % 40);
...
static void updateDrive(float dt) {
	...
	for (int i = 0; i < MAX_OBS; i++) if (obs[i].on) {
		...
		if (obs[i].z < 9.f && obs[i].z > 1.2f && fabsf(obs[i].lane - lane) < 0.75f) {
			crash++;
		...
		if (obs[i].z < 1.2f) obs[i].on = false;
	}
	if (dist >= target) {
...
static void proj(float ln, float z, float* x, float* y, float* sc) {
	if (z < 1.f) z = 1.f;
	float p = 280.f / z;
	*x = TOP_W * 0.5f + ln * 52.f * (p / 10.f);
	*y = 35.f + (TOP_H - 50.f) * (1.f - 1.f / (1.f + z * 0.05f));
	*sc = p * 0.9f; if (*sc > 95) *sc = 95; if (*sc < 4) *sc = 4;
}
...
static void drawGarage(float dt) {
	...
	snprintf(buf, sizeof(buf), "Mesh: %s  Tris: %d", meshOk?"OK":"FAIL", meshOk?carMesh.count/3:0);
...
static void drawDrive() {
	/* ROAD always first - bright and clear */
	C2D_TargetClear(top, C2D_Color32(100, 180, 245, 255));
	C2D_SceneBegin(top);
	rect(0, 0, TOP_W, 55, C2D_Color32(120, 190, 255, 255));
	rect(0, 55, TOP_W, TOP_H - 55, C2D_Color32(35, 140, 40, 255));
	for (int i = 18; i >= 0; i--) {
		float z1 = 1.2f + i * 2.9f, z2 = 1.2f + (i + 1) * 2.9f;
		float x1l,y1,s1,x1r,x2l,y2,s2,x2r;
		proj(-2.5f,z1,&x1l,&y1,&s1); proj(2.5f,z1,&x1r,&y1,&s1);
		proj(-2.5f,z2,&x2l,&y2,&s2); proj(2.5f,z2,&x2r,&y2,&s2);
		float midY = (y1+y2)*0.5f, hh = fabsf(y2-y1)+1.5f;
		float L = (x1l+x2l)*0.5f, RR = (x1r+x2r)*0.5f;
		u32 col = ((i + (int)(roadOff/14)) & 1) ? C2D_Color32(70,70,80,255) : C2D_Color32(50,50,60,255);
		rect(L, midY - hh*0.5f, RR-L, hh, col);
		if (((i + (int)(roadOff/9)) & 1) == 0)
			rect(TOP_W*0.5f - 4, midY - hh*0.35f, 8, hh*0.5f, C2D_Color32(255,230,40,255));
		rect(L - 10, midY - hh*0.5f, 10, hh, C2D_Color32(200,200,210,255));
		rect(RR, midY - hh*0.5f, 10, hh, C2D_Color32(200,200,210,255));
	}
	if (crashStun > 0.f)
		rect(0, 0, TOP_W, TOP_H, C2D_Color32(180, 20, 20, 80));
	...
	if (!meshOk) {
		...
		rect(TOP_W*0.5f-70, TOP_H-35, 140, 35, C2D_Color32(30,30,40,255));
	}
	...
	if (shaderOk && meshOk) {
		...
		for (int i = 0; i < MAX_OBS; i++) if (obs[i].on && obs[i].z < 75.f) {
			...
			float wx = rel * 1.15f;
			float wz = -3.2f - obs[i].z * 0.13f;
			float sc = 0.2f + 0.4f / (1.f + obs[i].z * 0.035f);
			drawCar3D(3.14159f, wx, -0.65f, wz, sc);
		}
		/* player hood facing FORWARD (pi) */
		float yaw = 3.14159f + steerV * 0.02f;
		if (crashStun > 0.f) yaw += spinVel * 0.3f;
		drawCar3D(yaw, 0.0f, -1.95f, -3.2f, 0.4f);
		C2D_Prepare();
	}

	C2D_TargetClear(bot, C2D_Color32(18,18,28,255)); C2D_SceneBegin(bot);
